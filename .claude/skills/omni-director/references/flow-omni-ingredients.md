...
## The "Character" feature is not a separate mechanism from `@` tagging — it's the same system

Flow's ingredient library lets you save an ingredient under one of (at least) three categories: **Character**, **Object**, or **Style**, either by uploading your own image or generating one with Imagen inside Flow. This category selection is not an alternative to `@` tagging — it's a label you attach *when adding* the ingredient, and `@filename` is still how you reference it inline in the prompt afterward. Sequence: **Add Ingredient → choose category (Character/Object/Style) → upload or generate the image → the ingredient is now attached to this prompt → reference it with `@filename` in the prompt text for role clarity.** An `@` tag with nothing added as an ingredient first resolves to nothing.

**Recommendation: tag people/creatures as `Character`, not a generic image upload.** The category almost certainly routes to the same kind of dedicated identity-preservation handling the raw API's `asset` reference type gets (vs. `style`, which is aesthetic-only) — so a person/creature added as `Character` should hold identity better across a sequence than the same image added without a category. Props get `Object`; a pure aesthetic/palette reference gets `Style`. Your own uploaded photos work fine for `Character` — you are not required to generate the character through Flow's Imagen tool first.

**Ingredient count: Google's own guidance says up to 3 ingredients per prompt for Ingredients-to-Video.** This is more concrete than this skill previously assumed — earlier drafts treated the cap as unconfirmed above ~4. Treat **3 as the real practical ceiling** for one call, not a soft suggestion. Any project needing a scene grid *plus* more than 2 recurring character/prop ingredients in the same call is over budget and needs a fallback: keep the grid (it carries the most information — staging, timing, and, since it was generated using the character references, a first-pass at their appearance too) and the single most drift-prone character/prop ingredient, and let the rest ride on the grid's own rendering plus prose identity descriptors rather than a dedicated ingredient slot. Confirm live whether 4 genuinely gets rejected or just silently under-weighted before assuming this cap is a hard wall — but design for 3 by default.
