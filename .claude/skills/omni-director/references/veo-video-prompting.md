...
- Base generation: **8 seconds** on standard Veo 3.1, resolutions `720p`/`1080p`/`4K` (4K in preview on some tiers), native synchronized audio. **[practitioner-reported, not independently verified against an official spec page]** Google's Omni model/mode is currently the exception: it supports single-clip generations up to **10 seconds**. If a project's target surface is ambiguous, confirm with the user which one they mean before assuming 8s vs. 10s — the two are not interchangeable and this skill defaults to the verified 8s figure when it isn't told otherwise.
