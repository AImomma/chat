...
- Video: **8-second base clips on standard Veo 3.1.** If the target is specifically Google's Omni model/mode, single-clip generations extend to **10 seconds** — ask the user which surface they're targeting if it's not already clear, and default to 8s when in doubt since that's the verified figure for standard Veo 3.1. Use timestamped sub-beats (`[00:00-00:02] ...`) to fit multiple `P##` beats in one clip; for sequences that need more than one clip, default to **independent calls** with matching `ATTACH` blocks at any cut point, and reserve Veo's extension mechanic (chains +7s per call, up to ~20 times, ~148s max, reusing the same reference plates) for genuinely continuous, uncut camera moves. See `references/veo-video-prompting.md`.
