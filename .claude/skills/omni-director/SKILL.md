...
- Video: 8-second base clips. Use timestamped sub-beats (`[00:00-00:02] ...`) to fit multiple `P##` beats in one clip; use Veo's extension mechanic (chains +7s per call, up to ~20 times, ~148s max, reusing the same reference plates) for sequences that need more than one clip. See `references/veo-video-prompting.md`.
...
- Every video prompt's `ATTACH AS ASSET:` / `ATTACH AS STYLE:` block lists real plate filenames, respects the ≤3 asset / ≤1 style caps per call, and every character appearing in that beat range has their plates attached.
